...
def check_constraints(individual,data):
    ...
    temp = np.vstack((genAmount,investedNum))

    individual =  temp.flatten().tolist()
    ...
    for year in range(16):
        for unitType in range(11):
            if birey[year][unitType] > capacities[year][unitType]:
                birey[year][unitType] = capacities[year][unitType]
                # nuclear üretim varsa ilk 11 yıl
                if year < 11 and unitType == 5 and birey[year][unitType] > 0:
                    birey[year][unitType] = 0
    ...
    for i in range(16):
        for unitType in range(11):
            if investedNum[i][unitType] > constructionLimit[unitType]:
                investedNum[i][unitType] = constructionLimit[unitType]
